  ...
  <v-app dark>
    ...
    <v-navigation-drawer v-model="drawer" fixed clipped app :width="150">
      ...
      <v-list dense>
        ...
        <v-list-tile to="/tree/">
          <v-list-tile-content>
            <v-list-tile-title> Item tree </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>

        <v-list-tile to="/">
          <v-list-tile-content>
            <v-list-tile-title> Item list </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>

        <v-list-tile to="/change-feed">
          <v-list-tile-content>
            <v-list-tile-title> Change feed </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
      </v-list>
      ...
      <v-list dense>
        <v-list-tile to="/entry/">
          <v-list-tile-content>
            <v-list-tile-title> New item </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
      </v-list>
      ...
      <v-subheader
        class="caption mt-3 grey--text text--darken-2 fade-out-legal"
      >
        &#169; 2022 Jacob Liscom <br />
        Version: {{ version }}
      </v-subheader>
    </v-navigation-drawer>
    ...
    <v-toolbar color="blue" dense fixed clipped-left app flat>
      ...
      />
      <v-toolbar-items>
        <v-btn flat @click.stop="search" style="min-width: 0"> Search </v-btn>
      </v-toolbar-items>
    </v-toolbar>
...
export default {
  ...
  methods: {
    resetSearchString(route) {
      // reset search string on route changes
      if (route.name !== 'search') {
        // except if they go to search
        this.searchString = '';
      } else {
