...
let Database = class {
   ...
   */
  constructor(databaseConfig, PouchDB, dbPath = 'StoreDown') {
    ...
    if (databaseConfig && databaseConfig.url) {
      ...
      databaseConfig.syncCallback(
        this.localDatabase.sync(this.remoteDatabase, databaseConfig.syncOptions)
      );
    }
  }
      ...
      this.localDatabase
        ...
        .then((data) => {
          data.rows = data.rows
            ...
            .forEach((item) => returnItems.push(item.doc));
...
export default function (forceCreate, replicationCompleteCallback) {
  if (forceCreate || !database) {
        ...
        syncObject
          ...
          .on('paused', function () {
            ...
            replicationTimeout = setTimeout(function () {
              ...
              if (replicationCompleteCallback) {
                replicationCompleteCallback();
              } // Let folks know
    ...
    if (userConfig && databaseConfig) {
      remoteDatabaseConfig.url = `${databaseConfig.protocol}://${
        userConfig.username
      }:${userConfig.password}@${databaseConfig.host}${
