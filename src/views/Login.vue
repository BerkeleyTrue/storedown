    ...
    <v-layout align-center justify-center wrap>
      <v-flex xs12>
        <v-alert :value="loggedIn" color="success">
          ...
          <v-btn depressed flat small @click.stop="logOut"
            >Click here to log out.</v-btn
          >
        </v-alert>
      </v-flex>
          ...
          <v-card-text>
            <v-form>
              ...
              <v-text-field
                ...
                label="Database name"
                placeholder="StoreDown"
                type="text"
              ></v-text-field>
              <v-switch
                v-model="database.remember"
                label="Remember database details?"
                hide-details
              ></v-switch>
              ...
              <v-switch
                ...
                label="Remember username and password?"
                :disabled="!database.remember"
                hide-details
              ></v-switch>
            </v-form>
          </v-card-text>
          <v-card-actions>
            <v-btn depressed small @click.stop="skip" color=""
              >Run in local/demo mode</v-btn
            >
            <v-spacer></v-spacer>
...
export default {
  data: () => ({
    ...
    loggedIn:
      ConfigService.getLocalConfig('user') &&
      ConfigService.getLocalConfig('database'),
    progressModal: false,
  }),
  watch: {
    'database.remember'(to, from) {
      // Make sure we stop skipping when the modal closes
      if (!to) {
