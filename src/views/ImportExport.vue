<template>
  <v-container grid-list-lg fluid>
        ...
        <v-card flat>
          ...
          <v-card-actions>
            <v-container fluid class="pt-0">
              <v-layout wrap>
                <v-flex xs12 sm6 xl4>
                  <v-btn depressed block @click.stop="deleteConfirm = true"
                    >Empty database</v-btn
                  >
                </v-flex>
              </v-layout>
            </v-container>
            ...
            <v-container fluid class="pt-0">
              <v-layout wrap>
                <v-form class="flex xs12">
                  <v-layout wrap>
                    <v-flex xs12 sm4>
                      <v-btn depressed block @click.stop="exportYML"
                        >Export YML</v-btn
                      >
                    </v-flex>
                    <v-flex xs12 sm4>
                      <v-btn depressed block @click.stop="exportJSON"
                        >Export JSON</v-btn
                      >
                    </v-flex>
                    <v-flex xs12 sm4>
                      <v-btn depressed block @click.stop="exportCSV"
                        >Export CSV</v-btn
                      >
                    </v-flex>
                  </v-layout>
                  ...
                  <v-layout wrap>
                    <v-flex xs12>
                      <v-textarea
                        ...
                        v-model="importText"
                        label="Import"
                        hint="YML or JSON paste"
                      ></v-textarea>
                    </v-flex>
                    <v-flex xs12 sm6>
                      <v-btn depressed block @click.stop="importYML"
                        >Import YML</v-btn
                      >
                    </v-flex>
                    <v-flex xs12 sm6>
                      <v-btn depressed block @click.stop="importJSON"
                        >Import JSON</v-btn
                      >
                    </v-flex>
                  </v-layout>
    ...
    <v-dialog
      ...
      width="500"
      :persistent="deleteInProgress"
    >
      <v-card>
        <template v-if="!deleteInProgress">
          <v-card-title class="headline">Delete</v-card-title>
          <v-card-text>
            Are you sure you want to delete all items in the database? This will
            also delete record on the database server.
          </v-card-text>
          <v-card-actions>
    ...
    <v-dialog
      :value="progressModalText.length > 0"
      width="500"
      :persistent="true"
    >
      <v-card>
  ...
  computed: {
    exportFileName: function () {
      ...
      if (databaseConfig && databaseConfig.database) {
        fileName = `${databaseConfig.database}.${timestamp}`;
      }
  ...
  methods: {
    ...
    importItems: function (items) {
      ...
      DatabaseService()
        ...
        .bulkDocs(items)
        .then((responses) => {
          // This function displays errors and provides user with error file.
          let importCount = 0;
          let errors = [];
    ...
    exportJSON: function () {
      ...
      DatabaseService()
        ...
        .then((data) => {
          saveAs(
            new Blob([JSON.stringify(data.rows)], {
              type: 'text/plain;charset=utf-8',
            }),
    ...
    exportYML: function () {
      ...
      DatabaseService()
        ...
        .then((data) => {
          saveAs(
            new Blob([yaml.safeDump(data.rows)], {
              type: 'text/plain;charset=utf-8',
            }),
            `${this.exportFileName}.yml`
          );
          this.progressModalText = '';
        });
    },
    ...
    exportCSV: function () {
      ...
      this.config.dataDefinition.forEach((group) => {
        group.fields.forEach((item) => {
          ...
          if (item.type === 'libraryStyleStatus') {
            objectColumns.push(item.name);
          }
        });
      });
      ...
      DatabaseService()
        ...
        .then((data) => {
          data.rows.forEach((item) => {
            // Stringify escape object field types
