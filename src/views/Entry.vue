  ...
  <v-container grid-list-lg fluid>
    <v-layout>
      ...
      <v-flex xs9 text-xs-right>
        ...
        >
          Duplicate </v-btn
        ><!-- TODO  add a quantity field that automatically insert that number of copies with different ids. Only for new items? -->
        <v-btn small color="success" depressed @click.stop="save"> Save </v-btn>

        <v-btn
          small
          color="error"
          depressed
          @click.stop="tempState.deleteConfirm = true"
        >
          Delete
        </v-btn>
      </v-flex>
    ...
    <v-layout wrap>
      <v-flex
        d-flex
        :class="[group.size ? `xs${group.size}` : 'xs12 lg6 xl4']"
        v-for="group in dataDefinition"
            ...
            <v-container fluid class="pt-0">
              <v-layout wrap>
                <v-form class="flex xs12">
                  <v-layout wrap>
                    <v-flex
                      :class="field.size ? `xs${field.size}` : 'xs12 sm6 xl4'"
                    ...
                    >
                      ...
                      <v-text-field
                        ...
                        :label="field.displayName"
                        :prefix="
                          field.prefixes[(item[field.name] || '').toLowerCase()]
                        "
                        :disabled="field.disabled"
                      ></v-text-field>
                        ...
                        >
                          <template v-slot:selection="data">
                            <v-chip
                              ...
                              close
                              @input="
                                item[field.name].splice(
                                  item[field.name].indexOf(data.item),
                                  1
                                )
                              "
                            >
                      ...
                      <v-menu
                        ...
                        v-model="fieldStates[field.name]"
                      ...
                      >
                        ...
                        <v-date-picker
                          v-model="item[field.name]"
                          @input="fieldStates[field.name] = false"
                        ></v-date-picker>
                      </v-menu>
                        ...
                        <v-layout>
                          <v-flex xs6>
                            <v-btn
                              ...
                              @click.stop="
                                toggleLibraryStyleStatus(field, true)
                              "
                              :disabled="getOrSetDefault(field, {}).status"
                              >Check In</v-btn
                            >
                          </v-flex>
                          <v-flex xs6>
                            <v-btn
                              ...
                              :color="field.color ? field.color : 'primary'"
                              @click.stop="
                                toggleLibraryStyleStatus(field, false)
                              "
                              :disabled="!getOrSetDefault(field, {}).status"
                              >Check Out</v-btn
                            >
      ...
      <v-card>
        ...
        <v-card-actions>
          <v-btn
            color=""
            depressed
            @click.stop="tempState.deleteConfirm = false"
          >
            Cancel
          </v-btn>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    save: function () {
      ...
      this.dataDefinition.forEach((group) =>
        group.fields
          .filter((field) => field.type === 'lastModified')
          .forEach(
            (field) => (this.item[field.name] = new Date().toISOString())
          )
      );
      ...
      this.dataDefinition.forEach((group) =>
        group.fields
          .filter((field) => field.type === 'lastDate')
          .forEach((field) => {
            let dates = [];
