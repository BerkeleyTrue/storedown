...
export default {
  ...
  methods: {
    ...
    loadData: function (skip = 0, limit = 100) {
      ...
      let buildTree = () => {
        ...
        return new Promise((resolve, reject) => {
          ConfigService.getConfigObject()
            .then((config) => {
              let i = 0;
              if (!findPathField(config)) {
                return reject(
                  new Error(
                    'Tree view not supported because a name field or field with type path is not set.'
                  )
                );
              }

              let pathFieldName =
                this.pathField.name.charAt(0).toUpperCase() +
                this.pathField.name.slice(1);
              let loopFunction = () => {
                DatabaseService()
                  .getAllItems({ skip: i, limit: RECORD_TO_PULL })
                  .then((data) => {
                    ...
                    data.rows.forEach((item) => {
                      let prefix =
                        this.pathField.prefixes[
                          (item[this.pathField.name] || '').toLowerCase()
                        ];
                      if (!prefix) {
                      ...
                      if (item[this.pathField.name]) {
                        path = `${prefix}${
                          item[this.pathField.name]
                        }`.toLowerCase(); // Lower case everything because computer science
                      }
      ...
      // Execute
      return new Promise((resolve, reject) => {
        if (this.tree._items.length) {
          return resolve(loadPrefix(this.prefix));
        } // If tree is already set Ether prefixes or
