              ...
              DatabaseService()
                ...
                .then((data) => {
                  ...
                  results = results.concat(
                    new Fuse(data.rows, fuseOptions).search(query)
                  );

                  let resultsToSkip = skipResults - resultsSkipped;
                  if (resultsToSkip > 0) {
                    resultsSkipped += Math.min(resultsToSkip, results.length);
                    if (resultsToSkip <= results.length) {
                      results = results.slice(resultsToSkip);
                    } else {
                      results = [];
                    }
                  }
                  ...
                  if (results.length < resultLimit && i < data.total_rows) {
                    return loopFunction();
                  }
                  ...
                  if (i + 10 < data.total_rows) {
                    totalRows += 1;
                  } // +1 So the table show there is more data
